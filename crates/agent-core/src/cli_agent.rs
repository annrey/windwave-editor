...
impl CliPlatform {
    ...
    pub fn install_instructions(&self) -> &str {
        match self {
            ...
            Self::OpenCode => "npm install -g opencode",
            Self::Custom(_) => "Ensure the CLI is on PATH",
        }
    }
}
...
impl CliAgent {
    pub fn new(id: AgentId, platform: CliPlatform, project_root: impl Into<String>) -> Self {
        ...
        Self {
            id,
            name: format!("cli-{}", platform.executable()),
            installed,
            platform,
    ...
    fn invoke(&self, instruction: &str) -> Result<String, String> {
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| format!("Failed to launch {}: {}", self.platform.executable(), e))?;
        if let Some(mut stdin) = child.stdin.take() {
            let _ = writeln!(stdin, "{}", instruction);
        }
        let out = child
            .wait_with_output()
            .map_err(|e| format!("{} error: {}", self.platform.executable(), e))?;
...
impl Agent for CliAgent {
    ...
    fn capabilities(&self) -> &[crate::registry::CapabilityKind] {
        &[
            crate::registry::CapabilityKind::CodeRead,
            crate::registry::CapabilityKind::CodeWrite,
            crate::registry::CapabilityKind::CodeGen,
    ...
    async fn handle(&mut self, request: AgentRequest) -> Result<AgentResponse, AgentError> {
            ...
            return Ok(AgentResponse {
                ...
                result: AgentResultKind::Failed {
                    reason: format!(
                        ...
                        self.platform.install_instructions()
                    ),
                },
                events: vec![],
            });
        ...
        match self.invoke(&request.instruction) {
            ...
            Err(err) => Ok(AgentResponse {
                agent_id: self.id,
                agent_name: self.name.clone(),
                result: AgentResultKind::Failed { reason: err },
                events: vec![],
            }),
        }
    }
...
mod tests {
    use super::*;
    #[test]
    fn test_platform_exe() {
        assert_eq!(CliPlatform::Claude.executable(), "claude");
    }
    #[tokio::test]
    async fn test_not_installed() {
        let mut a = CliAgent::new(
            AgentId(400),
            CliPlatform::Custom("nonexistent_xyz".into()),
            "/tmp",
        );
