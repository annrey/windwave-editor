//! Planner - 规则驱动的编辑计划生成器
//!
//! RuleBasedPlanner 将用户的自然语言请求（UserRequest）解析为结构化的 EditPlan，
//! 包含复杂度评估、风险评估、执行模式选择以及分步计划构建。
//!
//! # 核心流程
//! 1. 分析请求文本，估算复杂度和风险
//! 2. 根据复杂度/风险选择执行模式（Direct / Plan / Team）
//! 3. 解析中英文关键词，构建具体的编辑步骤
//!
//! # Planner trait
//! 支持双轨实现：`RuleBasedPlanner`（关键词匹配）和 `LlmPlanner`（LLM CoT 推理）。
...
/// 基于规则的编辑计划生成器
///
/// RuleBasedPlanner 通过分析用户请求中的关键词来决定：
/// - 任务的复杂度（Simple / Medium / Complex）
/// - 操作风险（Safe / LowRisk / MediumRisk / HighRisk / Destructive）
/// - 执行模式（Direct / Plan / Team）
/// - 具体的执行步骤
///
/// # 示例
/// ```ignore
...
///     task_id: 1,
///     available_tools: vec!["create_entity".into()],
///     scene_entity_names: vec!["Player".into()],
/// };
/// let plan = planner.create_plan("创建一个红色敌人", 1, context);
...
// ============================================================================
// LlmPlanner — LLM CoT 驱动的智能规划器
// ============================================================================

/// LLM-powered plan generator using Chain-of-Thought reasoning.
///
/// Sends a structured CoT prompt to the LLM, parses the JSON response
/// into an `EditPlan`, and falls back to `RuleBasedPlanner` if the LLM
...
impl LlmPlanner {
    ...
    fn build_cot_prompt(&self, request: &str, context: &PlannerContext) -> Vec<crate::llm::LlmMessage> {
        ...
        // Inject memory context into the layered context if available
...
impl Planner for LlmPlanner {
    fn create_plan(
        ...
        if llm_ready {
            ...
            let model = "gpt-4o-mini".to_string(); // TODO: configurable
            let request = crate::llm::build_chat_request(model, messages);

            // Sync bridge to async LLM call
            ...
            match result {
                Ok(response) => {
                    ...
                    // LLM returned empty steps or unparseable — fallback
                }
                Err(_) => {
