...
use crate::memory::{EpisodeType, MemorySystem, MemoryTier};
...
impl EventMemoryBridge {
    ...
    pub fn process_event(&mut self, event: &EditorEvent, memory: &mut MemorySystem) {
        ...
        match event {
            EditorEvent::EditPlanCreated {
                ...
                let intent = format!(
                    "Executing plan: {} ({} steps, risk={})",
                    title, steps_count, risk
                );
                memory.working.set_intent(&intent);
                let plan_id = memory.episodic.record_plan(title, *steps_count);
                eprintln!(
                    "[EventBridge] PlanCreated -> L3 intent + L2 episode(id={:?})",
                    plan_id
                );
            }
            ...
            EditorEvent::StepCompleted { title, result, .. } => {
                ...
                if let Some(entity_name) = Self::extract_entity_from_text(result) {
                    ...
                    if self.recent_entity_names.len() > 10 {
                        self.recent_entity_names.remove(0);
                    }
                    memory.working.register_entity(
                        &entity_name,
                        crate::types::EntityId(self.entities_discovered as u64),
                    );
                ...
                memory
                    .procedural
                    .observe_decision(&step_ctx, "completed", &outcome, true);
            ...
            EditorEvent::StepFailed { title, error, .. } => {
                ...
                memory.episodic.record_error(
                    error,
                    Some(serde_json::json!({"context": format!("Step '{}' failed", title)})),
                );
                ...
                memory
                    .procedural
                    .observe_decision(&step_ctx, "failed", error, false);
                self.patterns_learned += 1;
            }
            ...
            EditorEvent::DirectExecutionStarted { request, .. } => {
                ...
                if let Some(pref) = self.extract_preference(request) {
                    ...
                    memory
                        .working
                        .add_hint(&format!("Preference detected: {}={}", pref.0, pref.1));
                }
            }
            ...
            EditorEvent::DirectExecutionCompleted { request, success } => {
                if *success {
                    ...
                    memory
                        .episodic
                        .record_error(&format!("Direct execution failed for: {}", request), None);
                }
            }
            ...
            EditorEvent::ExecutionCompleted { success, .. } => {
                ...
                memory.working.add_hint(&hint);
                if *success && memory.procedural.workflow_count() > 0 {
    ...
    fn extract_entity_from_text(text: &str) -> Option<String> {
        ...
        for pat in &patterns {
            if let Ok(Some(caps)) = regex_lite_match(pat, text) {
                if !caps.is_empty() {
                    return Some(caps[0].clone());
                }
            }
        }
        let name_patterns = [
            "Player", "Enemy", "Boss", "NPC", "Wall", "Floor", "Camera", "UI",
        ];
...
fn regex_lite_match(pattern: &str, text: &str) -> Result<Option<Vec<String>>, ()> {
    ...
    while i < pat_bytes.len() {
        match pat_bytes[i] {
            ...
            c => {
                if i < txt_bytes.len() && txt_bytes[i] == c {
                    i += 1;
                } else {
                    return Ok(None);
                }
...
mod event_bridge_tests {
    ...
    fn test_process_execution_completed_success() {
        let mut bridge = EventMemoryBridge::new();
        let mut memory = MemorySystem::default();
        let event = EditorEvent::ExecutionCompleted {
            plan_id: "plan-001".into(),
            success: true,
        };
        bridge.process_event(&event, &mut memory);
    ...
    fn test_process_batch() {
        ...
        let events = vec![
            ...
            EditorEvent::ExecutionCompleted {
                plan_id: "p1".into(),
                success: true,
            },
        ];
