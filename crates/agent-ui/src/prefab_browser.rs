...
use agent_core::bevy_editor_model::{
    ComponentPatch, PrefabDefinition, PrefabId, PrefabNode, PrefabRegistry as CorePrefabRegistry,
};
use bevy::prelude::*;
...
pub struct PrefabRegistry {
    pub inner: CorePrefabRegistry,
}
impl PrefabRegistry {
    pub fn register(&mut self, p: PrefabDefinition) {
...
#[derive(Message, Debug, Clone)]
pub struct InstantiatePrefabEvent {
...
impl Plugin for PrefabBrowserPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<PrefabBrowserState>()
            ...
            .add_message::<InstantiatePrefabEvent>()
            .add_systems(EguiPrimaryContextPass, render_prefab_browser)
...
fn render_prefab_browser(
    ...
    mut state: ResMut<PrefabBrowserState>,
    prefab_registry: Option<Res<PrefabRegistry>>,
    mut create_events: Option<MessageWriter<CreatePrefabEvent>>,
    mut instantiate_events: Option<MessageWriter<InstantiatePrefabEvent>>,
    ...
    if !layout_mgr.is_visible("prefab_browser") {
        return;
    }
    ...
    egui::Window::new("Prefab Browser")
        ...
        .show(ctx, |ui| {
                ...
                if items.is_empty() {
                    ...
                    for p in &items {
                        ...
                        if ui
                            .selectable_label(
                                sel,
                                format!("{} ({} comps)", p.name, p.component_count),
                            )
                            .clicked()
                        {
            ...
            if let Some(id) = &state.selected_prefab {
                if let Some(prefab) = prefab_registry
                    .as_ref()
                    .and_then(|r| r.get(&PrefabId(id.clone())))
                {
                    ui.heading(&prefab.name);
                    if ui.button("Instantiate in Scene").clicked() {
                        if let Some(ref mut writer) = instantiate_events {
                            writer.write(InstantiatePrefabEvent {
                                prefab_id: id.clone(),
                            });
                        }
                    }
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Create").clicked() && !state.new_prefab_name.is_empty() {
                        if let Some(ref mut writer) = create_events {
                            writer.write(CreatePrefabEvent {
                                name: state.new_prefab_name.clone(),
                            });
                        }
...
fn handle_create_prefab(
    mut events: Option<MessageReader<CreatePrefabEvent>>,
    ...
    for event in events.iter_mut().flat_map(|r| r.read()) {
        if let Some(entity) = editor_selection.selected_entity {
            ...
            log::info!(
                "Prefab create requested: {} for entity {:?}",
                event.name,
                entity
            );
...
fn handle_instantiate_prefab(
    mut events: Option<MessageReader<InstantiatePrefabEvent>>,
...
fn spawn_prefab_cmd(commands: &mut Commands, node: &PrefabNode) -> Entity {
    ...
    if !node.name.is_empty() {
        cmd.insert(Name::new(node.name.clone()));
    }
...
fn apply_patch_to_cmd(cmd: &mut EntityCommands, patch: &ComponentPatch) {
    ...
    match patch.type_name.as_str() {
        ...
        "Sprite" => {
            let r = f32_from(p, "color_rgba", 0, 1.0);
            let g = f32_from(p, "color_rgba", 1, 1.0);
            let b = f32_from(p, "color_rgba", 2, 1.0);
            let a = f32_from(p, "color_rgba", 3, 1.0);
            let sz = p.get("custom_size").and_then(|v| v.as_array()).map(|a| {
                Vec2::new(
                    a.first().and_then(|x| x.as_f64()).unwrap_or(50.0) as f32,
...
fn f32_from(
    p: &std::collections::HashMap<String, serde_json::Value>,
    key: &str,
    idx: usize,
    def: f32,
) -> f32 {
...
pub fn toggle_prefab_browser(state: &mut ResMut<PrefabBrowserState>) {
