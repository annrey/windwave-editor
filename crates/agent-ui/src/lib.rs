...
pub mod open_world_interaction;
pub mod open_world_quest_panel;
...
pub mod world_timeline_panel;

pub use agent_config_panel::{
    set_llm_status, toggle_agent_config, update_token_usage, AgentConfigPanelPlugin,
    AgentConfigState, TokenUsageDisplay,
};
pub use ai_frameworks_panel::{
    toggle_ai_frameworks_panel, AIFrameworksPanelPlugin, AIFrameworksState,
};
pub use asset_browser::{
    refresh_assets, toggle_asset_browser, AssetBrowserPlugin, AssetBrowserState, AssetEntry,
    AssetType,
};
pub use console_panel::{log_console, ConsolePanelPlugin, ConsoleState, LogLevel};
pub use debug_panel::{toggle_debug_panel, DebugPanelPlugin, DebugPanelState, DebugTab};
pub use diff_preview::{
    create_change_set_from_description, hide_diff_preview, show_diff_preview, ChangeKind,
    DiffPreviewPlugin, DiffPreviewState, ExpectedChange,
};
pub use director_desk::{DirectorDeskPlugin, DirectorDeskState, PendingApprovalInfo, UserAction};
pub use editor_selection::{
    selection_shortcuts, EditorSelection, EditorSelectionPlugin, SelectionChangedEvent,
    SelectionContext,
};
pub use game_input::{
    GameInputAction, GameInputEvent, GameInputPlugin, InputMapping, InputStateBuffer,
};
pub use gizmo::{get_gizmo_mode_description, set_gizmo_mode, GizmoMode, GizmoPlugin, GizmoState};
...
pub use interactable::{
    Interactable, InteractablePlugin, InteractableState, InteractionCompleteEvent,
    InteractionPrompt, InteractionRadius, InteractionType,
};
pub use layout::{LayoutCommand, LayoutDefinition, LayoutManager, PanelConfig, PanelPosition};
pub use memory_persistence::{
    default_memory_path, load_persistent_memory, save_persistent_memory, MemoryPersistenceConfig,
};
pub use open_world_interaction::{
    drain_open_world_interaction_commands, enqueue_open_world_interaction_commands,
    OpenWorldInteractionBinding, OpenWorldInteractionBindings, OpenWorldInteractionCommand,
    OpenWorldInteractionCommandQueue,
};
pub use open_world_quest_panel::{
    sync_open_world_quest_panel_from_timeline_replay, toggle_open_world_quest_panel,
    OpenWorldQuestObjectiveRow, OpenWorldQuestPanelPlugin, OpenWorldQuestPanelState,
    OpenWorldQuestRow,
};
pub use permission_panel::{toggle_permission_panel, PermissionPanelPlugin, PermissionState};
pub use play_mode::{is_play_mode, PlayModePlugin, PlayModeState};
pub use prefab_browser::{
    create_prefab_from_entity, toggle_prefab_browser, PrefabBrowserPlugin, PrefabBrowserState,
    PrefabEntry, PrefabRegistry,
};
pub use project_panel::{
    open_project_panel, show_project_info_bar, toggle_project_panel, ProjectPanelPlugin,
    ProjectPanelState, ProjectUiConfig,
};
pub use runtime_agent_panel::{
    toggle_runtime_agent_panel, RuntimeAgentPanelPlugin, RuntimeAgentPanelState,
};
pub use scene_panel::{SceneAwarePanelPlugin, SceneEntityInfo, ScenePanelState, SceneSummary};
pub use shortcuts::{ShortcutState, ShortcutsPlugin};
pub use skill_panel::{
...
pub use task_panel::{
    ...
    TaskPanelBackendTransaction, TaskPanelCommand, TaskPanelCommandResult, TaskPanelPlugin,
    TaskPanelSnapshot, TaskPanelState, TaskPanelSystemSet, TaskStatus,
};
...
pub use visual_understanding::{
    GoalCheckResult, VgrcCycleSummary, VisualAnalysis, VisualUnderstandingPlugin,
    VisualUnderstandingState,
};
pub use world_timeline_panel::{
    apply_world_timeline_updates, process_open_world_qa_requests, toggle_world_timeline_panel,
    OpenWorldQaArtifactPaths, OpenWorldQaRequestQueue, WorldTimelinePanelPlugin,
    WorldTimelinePanelState, WorldTimelineUpdateQueue,
};

use agent_core::{AgentIdentity, AgentStatus, BaseAgent, Message, MessageType};
use bevy::prelude::*;
...
impl Plugin for AgentUiPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(EguiPlugin::default())
            ...
            .add_plugins(AIFrameworksPanelPlugin)
            .add_plugins(task_panel::TaskPanelPlugin)
            .add_plugins(scene_panel::SceneAwarePanelPlugin)
            .add_plugins(skill_panel::SkillPanelPlugin)
            .add_plugins(WorldTimelinePanelPlugin)
            .add_plugins(OpenWorldQuestPanelPlugin)
            .add_plugins(bevy_adapter::ScreenshotPlugin)
            .add_plugins(GameInputPlugin)
            .add_plugins(InteractablePlugin)
            ...
            .add_systems(Update, update_agent_state)
            .add_systems(EguiPrimaryContextPass, render_agent_panel)
            .add_systems(Update, process_layout_commands)
            .add_systems(Update, enqueue_open_world_interaction_commands)
            .add_systems(EguiPrimaryContextPass, render_layout_settings)
            .add_systems(Update, selection_shortcuts);
    }
}

fn update_agent_state(runtime: Option<Res<AgentRuntime>>, mut chat_state: ResMut<ChatState>) {
...
fn render_agent_header(ui: &mut egui::Ui, chat_state: &ChatState) {
    ...
    ui.horizontal(|ui| {
        ui.add_sized(
            [44.0, 44.0],
            egui::Button::new("\u{25C8}").fill(indigo).corner_radius(10),
        );
        ui.vertical(|ui| {
...
fn render_user_message(ui: &mut egui::Ui, message: &Message) {
    ...
    ui.with_layout(egui::Layout::right_to_left(egui::Align::TOP), |ui| {
        ui.add_sized(
            [32.0, 32.0],
            egui::Button::new("\u{263A}").fill(blue).corner_radius(8),
        );
        ui.vertical(|ui| {
...
fn render_input_area(ui: &mut egui::Ui, chat_state: &mut ChatState) {
    ...
    ui.horizontal(|ui| {
        ...
        if (send_btn.clicked() || enter_pressed) && !chat_state.input_text.is_empty() {
    ...
    ui.horizontal_wrapped(|ui| {
        let actions = [
            "\u{4FEE}\u{590D}\u{8B66}\u{544A}",
            "\u{4F18}\u{5316}\u{6027}\u{80FD}",
            "\u{751F}\u{6210}\u{6D4B}\u{8BD5}",
            "\u{89E3}\u{91CA}\u{4EE3}\u{7801}",
        ];
...
fn process_layout_commands(
    ...
    for cmd in cmds {
        ...
        chat_state.add_message(Message::new_agent(format!("[Layout] {}", description)));
    }
}
